...
def generate(bundle_name: str = "alfie_blues_lookback_rnn-20200616.mag",
             sequence_generator = melody_rnn_sequence_generator,
             generator_id: str = "lookback_rnn",
  ...
  # on the last bar.
  generation_start_time = primer_end_time + 0.00001
  ...
  midi_filename = "%s_%s_%s_%s.mid" % (generator_name, generator_id,
                                    date_and_time, num)
...
def app(unused_argv):
  ...
  generate(
    "bundles/bill_evans_polyphony_rnn.mag",
    melody_rnn_sequence_generator,
    "bundles/polyphony_rnn",
    primer_filename="primers/two-bar/1/ArtPepper_BluesForBlanche_FI_1.midi",
    total_length_steps=192,
    temperature=1.1)
